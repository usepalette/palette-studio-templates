...
Scaffold `accounts/<name>/` from `accounts/_template/` and add it to the status board — so a new customer folder is correctly stamped, complete, and registered, instead of hand-copying (which drifts: missing files, unstamped frontmatter, a folder that never makes it onto `status.md`). The account companion to `/create-space` and `/new-person`.
